...
function [n, qVxyz_full, xyz_full] = load_n_qVxyz_xyz(path_data, filename)
    ...
    if isfile([output_path_tmp, name, '_tmp.mat'])
        load([output_path_tmp, name, '_tmp.mat'], 'qVxyz_full', 'xyz_full');  %  файл есть
        ...
        if (~isfile([output_path_tmp, name, '_tmp.mat']))
            save([output_path_tmp, name, '_tmp.mat'], 'qVxyz_full', 'xyz_full');  %  сохранение данных для ускорения
            fprintf('\t%s\n\t%s\n\t%s\n', datestr(datetime(now, 'ConvertFrom', 'datenum')), 'Файл для ускорения записан по адресу:', append(output_path_tmp, name, '_tmp.mat'));
        end
    end
