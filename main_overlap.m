...
file = 'w3_1.irc';  %  имя файла
L = 4000;  %  длина интервала, по которому считаем вектор, измеряется в отсчетах
diff = 6000;  %  расстояние между первым и вторым интервалом
...
T = fix(1 + (length(E12)-(diff+L))/step);
A = zeros(T, 3*N, 3*N);
for t = 1:T
...
draw_overlap_video(A, L, diff, step, path_video);