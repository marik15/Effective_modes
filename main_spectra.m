...
path_data = 'D:\MATLAB\Эффективные моды\data\';  %  папка с файлами
files = {'w3_1.irc'};  %  файлы

modes = [2:3];  %  интересующие моды, диапазон можно задать так, например: 1:10
t1 = 1;  %  начало траектории, отсчеты
t2 = 5000;  %  конец траектории, отсчеты, либо слово 'end', если нужно посчитать до конца файла, но число строк неизвестно
t_step = 5000;  %  шаг, отсчеты, либо число 0, чтобы посчитать всю траекторию целиком

xlimit = 4000;  %  верхняя граница по частоте, см^-1, задаётся апостериори
k_mean = 11;  %  во сколько раз сжать вейвлет-спектр (ускорить работу)
