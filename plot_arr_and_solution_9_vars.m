...
LOAD = load(append('D:\MATLAB\Эффективные моды\scripts\Result 9_vars 6 order step ', num2str(step), ' all.mat'));
path_aux = 'D:\MATLAB\Эффективные моды\Вспомогательные файлы\';
path_output = append('D:\MATLAB\Эффективные моды\Графики решений уравнения 6 порядка шаг ', num2str(step), ' вариация начальных условий\');
...
for file_id = 1:numel(names)
    ...
    [t_de, sol] = ode15s(@(t, y) odefun_kin(t, y, k_best), linspace(t(1), t(end), const), LOAD.y0_best{file_id}(top, :));
