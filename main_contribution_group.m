...
path_output1 = 'E:\MATLAB\Эффективные моды\raw data\Отдельно каждая группа\';
...
for files_id = 1:size(files_group, 1)  %  [1:11, 13, 15:16, 18]  %  1:size(files_group, 1)  %  [12, 14, 17, 19]
    ...
    for sub_file_id = 1:size(files_group{files_id}, 2)
        ...
        N_steps = fix(1 + (size(qVxyz_full, 1) - N_width)/step);  %  количество позиций
        ...
        for k = 1:size(range, 1)
            %plt = plot(ax, (1:N_steps)*step/fs*(1e+12), arr(:, k), 'LineWidth', 0.6, 'Marker', markers{sub_file_id}, 'LineStyle', '-', 'MarkerSize', 6, 'Color', colors{k}, 'MarkerFaceColor', colors{k});
            plt = plot(ax, 1:N_steps, arr(:, k), 'LineWidth', 0.6, 'Marker', markers{sub_file_id}, 'LineStyle', '-', 'MarkerSize', 6, 'Color', colors{k}, 'MarkerFaceColor', colors{k});
            p = [p, plt];  %#ok<*AGROW>
            lbl = append(name(1:end-4), ' ', keywords{k}, ' ', num2str(range(k, 1)), '-', num2str(range(k, 2)));
            labels = [labels, lbl];
        end
        %  raw_data(:, (0:2) + (3 * sub_file_id - 2)) = arr;

        %%{
        xlim(ax, [0, N_steps]);
        title(ax, 'Оценка вкладов областей в спектр', 'Interpreter', 'None');
        xlabel(ax, append('Время, номер шага по ', num2str(step), ' отсчетов'));
        ylabel(ax, 'Доля');
        l = legend(ax, p, labels, 'Location', 'north', 'Interpreter', 'None');
        box(ax, 'on');
        grid(ax, 'on');
        set(ax, 'FontSize', 24);
        myfittype1 = fittype("C * exp(k * (t - t0))", dependent = "y", independent = "t", coefficients = ["C", "k", "t0"]);
        myfittype2 = fittype("C * (1 - exp(k * (t - t0)))", dependent = "y", independent = "t", coefficients = ["C", "k", "t0"]);
        %exportgraphics(fig, append(path_output1, files_group{files_id}{sub_file_id}(1:end-4), ', порог ', num2str(threshold), ', доли областей.png'), "Resolution", 300);
        close(fig);
        %}}
    end
    %  write_matrix(append(path_output1, files_group{files_id}{1}(1:end-4), ' + ', files_group{files_id}{2}(1:end-4), ' + ', files_group{files_id}{3}(1:end-4), ', 50 пс, 3 графика, шаг ', num2str(step), ', порог ', num2str(threshold), ', доли областей.txt'), [(((1:N_steps) - 0.5)*step/fs*(1e+12))', raw_data], '%4.4f ', 'w');
    %write_matrix(append(path_output1, files_group{files_id}{1}(1:end-4), ', 50 пс, 3 графика, шаг ', num2str(step), ', порог ', num2str(threshold), ', доли областей.txt'), [(((1:N_steps) - 0.5)*step/fs*(1e+12))', raw_data], '%4.4f ', 'w');
    ...
    myfittype1 = fittype("C * exp(k * (t - t0))", dependent = "y", independent = "t", coefficients = ["C", "k", "t0"]);
    myfittype2 = fittype("C * (1 - exp(k * (t - t0)))", dependent = "y", independent = "t", coefficients = ["C", "k", "t0"]);
    %saveas(fig, append(path_output1, files_group{files_id}{1}(1:end-4), ' + ', files_group{files_id}{2}(1:end-4), ' + ', files_group{files_id}{3}(1:end-4), ' по ', num2str(size(range, 1)), ' графика, порог ', num2str(threshold), ', доли областей.png'));
    %exportgraphics(fig, append(path_output1, files_group{files_id}{1}(1:end-4), ' + ', files_group{files_id}{2}(1:end-4), ' + ', files_group{files_id}{3}(1:end-4), ', 9 графиков, порог ', num2str(threshold), ', доли областей.png'), "Resolution", 300);
    close(fig);
    %}
    %write_matrix(append('E:\MATLAB\Эффективные моды\raw data\20240912\', files_group{files_id}{1}(1:end-4), ' ', num2str(step), ' 50 пс.txt'), [((1:N_steps)*step/fs*(1e+12))', arr], '%4.4f ', 'w');
end